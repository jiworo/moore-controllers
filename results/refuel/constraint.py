...
class CustomConstraint(Constraint):
    ...
    def register_arguments(self, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument("--nr-atoms", type=int, help="Maximum predicate length", default=-1)
    ...
    def build_constraint(
        ...
        fixed_idxs = set()  # Don't transition on fixed actions; empty delta for MC
        ...
        for state_idx in range(transition_matrix.nr_columns):
            ...
            if state_idx in fixed_idxs:  # Fixed case
        ...
        if self.has_predicates or self.has_out_degree:
            supp_delta = z3.Function("supp_delta", Q, Act, z3.SetSort(Q))  # => { q' in supp(delta(q,⋅,⋅)) }
            constrain_delta.extend(
        ...
        if self.has_out_degree:
            constrain_delta.extend(  # Out-degree
                z3.PbLe(
